...
export const getbandsDetail = (bandId: number) =>
  axios
    .get(`http://144.24.81.195:8080/api/emotion-bands/${bandId}/detail`)
    .then((Response) => {
...
export const getComments = async (
  ...
  try {
    const response = await axios.get<Comment[]>(
      `http://144.24.81.195:8080/api/emotion-bands/${emotionBandId}/comments`
    );
    return response.data;
  } catch (error) {
...
export const archiveBand = (emotionBandId: number, memberId: number) => {
  ...
  return axios
    .post(
      `http://144.24.81.195:8080/api/emotion-bands/${emotionBandId}/archive`,
      payload
    )
    .then((response) => {
      console.log(response.data);
...
export const dummyBandsDetail = () => {
  const dummyData = {
    ...
    emotion: "해커톤",
    description:
      "오늘 비가 와서 문득 떠오른 사람이 있어요. 같은 마음인 분들과 함께 듣고 싶어요",
